...
/// A widgets that controls how the [Slidable] is dismissed.
///
/// The [Slidable] widgets calls the [onDismissed] callback either after its size has
...
class SlidableDismissal extends StatelessWidget {
  /// Creates a widgets that controls how the [Slidable] is dismissed.
  ...
  /// Specifies if the widgets can be dismissed by sliding.
  ///
  /// Setting to false makes the widgets dismissible only by
  ...
  /// Called when the widgets has been dismissed, after finishing resizing.
  final DismissSlideActionCallback? onDismissed;

  /// Called before the widgets is dismissed. If the call returns false, the
  /// item will not be dismissed.
  ///
  /// If null, the widgets will always be dismissed.
  ...
  /// Called when the widgets changes size (i.e., when contracting before being dismissed).
  final VoidCallback? onResize;

  /// The amount of time the widgets will spend contracting before [onDismissed] is called.
  ///
  /// If null, the widgets will not contract and [onDismissed] will be called
  /// immediately after the widgets is dismissed.
  final Duration? resizeDuration;

  /// Defines the end offset across the main axis after the card is dismissed.
  ///
  /// If non-zero value is given then widgets moves in cross direction depending on whether
  /// it is positive or negative.
  final double crossAxisEndOffset;

  /// The widgets to show when the [Slidable] enters dismiss mode.
  final Widget child;
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        return data.overallMoveAnimation.value > data.totalActionsExtent ? child! : data.slidable.actionPane;
      },
      child: child,
    );
  }
              ...
              return Positioned.fill(
                child: Stack(
                  children: List.generate(data.actionCount, (index) {
                    ...
                    final displayIndex = data.showActions ? data.actionCount - index - 1 : index;
                    ...
                    return data.createFractionallyAlignedSizedBox(
                      ...
                      child: data.actionDelegate!
                          .build(context, displayIndex, data.actionsMoveAnimation, data.renderingMode),
                    );
                  }),
                ),
              );
