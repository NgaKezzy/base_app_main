...
/// Signature for determining whether the widgets will be dismissed for the
/// given [actionType].
///
...
/// so that the animation can be passed down to the final widgets.
///
/// See also:
///
///  * [SlideActionListDelegate], which is a delegate that has an explicit list
...
class SlidableData extends InheritedWidget {
  ...
  /// The direction in which this widgets can be slid.
  ...
  /// Whether the framework should notify widgets that inherit from this widgets.
  @override
...
/// A widgets that can be slid in both direction of the specified axis.
///
/// If the direction is [Axis.horizontal], this widgets can be slid to the left or to the right,
/// otherwise this widgets can be slid up or slid down.
///
/// By sliding in one of these direction, slide actions will appear.
class Slidable extends StatefulWidget {
  /// Creates a widgets that can be slid.
  ///
  /// The [actionsRight] contains the slide actions that appears when the child has been dragged down or to the right.
  /// The [actionsRight] contains the slide actions that appears when the child has been dragged up or to the left.
  ///
  ...
  Slidable({
    Key? key,
    required Widget child,
    required Widget actionsPane,
    List<Widget>? actionsLeft,
    List<Widget>? actionsRight,
    ...
    double? fastThreshold,
          ...
          key: key,
          child: child,
          actionPane: actionsPane,
          actionDelegate: SlideActionListDelegate(actions: actionsLeft),
          secondaryActionDelegate:
              SlideActionListDelegate(actions: actionsRight),
    ...
    setState(() {
      ...
      if (_actionCount > 0) {
        if (_dismissible && !widget.dismissal!.dragDismissible) {
          // If the widgets is not dismissible by dragging, clamp drag result
          // so the widgets doesn't slide past [_totalActionsExtent].
          ...
          if (_resizeAnimation != null) {
            ...
            assert(() {
              if (_resizeAnimation!.status != AnimationStatus.forward) {
                ...
                throw FlutterError(
                    'A dismissed Slidable widgets is still part of the tree.\n'
                    'Make sure to implement the onDismissed handler and to immediately remove the Slidable\n'
                    'widgets from the application once that handler has fired.');
              }
              return true;
            }());
